...
def main():
    ...
    root_path = pathlib.Path('H:/arty/data/IT-GAN')  #Todo: modify path
    data_path = root_path / 'data' #Todo: modify path
    print('data path: ', data_path)
    checkpoint_path = root_path / 'checkpoints' #Todo: modify path
    ...
    weight_path = os.path.join(checkpoint_path, 'G_Pretrained_%s_exp%d.pth' % (args.dataset, exp)) #Todo: modify path
    ...
    save_path = root_path / 'results' / 'EfficientGAN' #Todo: modify path
    ...
    accs_dict = {mode: [] for mode in mode_all}
    ...
    for mode in mode_all:
        if mode == 'real':
            ...
            def load_batch(idx):
                # img = generate(z_all[idx], labels_all[idx])
                img = images_all[idx]
                lab = labels_all[idx]
                return img.detach(), lab.detach()
            ...
            ''' load Efficient z '''
            fpath = os.path.join(checkpoint_path, 'EfficientGAN_final_%s_ConvNet_lrz0.001_exp%d.pt' % (args.dataset, exp)) #Todo: modify path
            print('use EfficientGAN vectors: %s' % fpath)
            data_z = torch.load(fpath, map_location=device)
            z_eff_all = deepcopy(data_z['z_eff_all']).detach()
        ...
        for eval_exp in range(args.num_evalnet):
            ...
            for ep_eval in range(args.Epoch_evaltrain + 1):
                ...
                if ep_eval % 1 == 0 or ep_eval == args.Epoch_evaltrain:
                    loss_test, acc_test = epoch('test', testloader, net, optimizer_net, criterion, args, flag_print=False)
    ...
    for mode in accs_dict.keys():
        print('%d %s acc = %.2f +- %.2f all = %s |%s evalute '%(len(accs_dict[mode]), args.model_eval, np.mean(accs_dict[mode])*100, np.std(accs_dict[mode])*100, accs_dict[mode], mode ))
