...
def main():
    ...
    for i in range(10):
        ...
        # extra = []
        # for j, ood_detector in enumerate(ood_detectors):
        #     ood_detector.to(device)
        #     real_mean_embedding_score = ood_detector.predict(real_emb[i].mean(dim=0).unsqueeze(0), batch_size=1, verbose=False)
        #     gen_embedding_score = ood_detector.predict(gen_emb[i].mean(dim=0).unsqueeze(0), batch_size=1, verbose=False)
        #     ood_detector.to("cpu")
        #     extra.append((f"  {j} Real Mean", real_mean_embedding_score))
        #     extra.append((f"  {j} Gen Mean", gen_embedding_score))
        #     print(f"Real Mean {j}: {real_mean_embedding_score}")
        #     print(f"Gen Mean {j}: {gen_embedding_score}")

        

        plot(scores_real[i][i].numpy(), scores_gen[i][i].numpy(), ood_scores, dataset, f"disp_plot_{ood_detectors[0].name}_{encoder_name}_{generator_name}_{i}", names=names)
        samples = 10
        ...
        for j in range(samples):
            ...
            ax[1, j].axis('off')
        plt.savefig(f"figs/{dataset}/sample_{ood_detectors[0].name}_{encoder_name}_{generator_name}_{i}.svg")
