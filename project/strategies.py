...
class EilertsenEscape():
    def __init__(self,
        ...
        self.optimizer = torch.optim.SGD(learnable, lr=lr, weight_decay=wd)
    ...
    def __call__(self,  verbose=False):
        ...
        if verbose:
            data_iter = tqdm.tqdm(self.generator, desc="anchor")
        else:
...
class LatentInversion():
    def __init__(self,
        ...
        self.model = model
        self.original = [p.detach().clone() for p in learnable]
        self.optimizer = torch.optim.Adam(learnable, lr=lr, weight_decay=wd)
        self.criterion = criterion_classifier
        self.reset_every = reset_every
        self.intermediate = None
        self.cache_intermediate = cache_intermediate
    ...
    def __call__(self,  verbose=False):
        ...
        for x, y, i in data_iter:
            ...
            loss = self.criterion(emb, real_emb)
            loss.backward()
